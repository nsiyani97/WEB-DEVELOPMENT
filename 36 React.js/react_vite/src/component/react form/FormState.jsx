...
function FormState() {
  ...
  function saveData(e) {
    ...
    if (id !== "") {
      const updated = data.map((item, index) => (index === id ? { name, age, email } : item));
      setData(updated);
      setId("");
    } else {
  ...
  return (
    <div>
      <form onSubmit={saveData}>
        <input value={name} onChange={(e) => setName(e.target.value)} type="text" placeholder="Enter a Name" />
        <input value={age} onChange={(e) => setAge(e.target.value)} type="number" placeholder="Enter an Age" />
